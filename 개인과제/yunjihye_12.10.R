...
yp<-data.frame(year,pop)
yp

ggplot(data=yp,mapping = aes(x=year,y=pop))+
  geom_line(col="green")
...
treemap(us1,
        index = c("state.division","st"),
        vSize = "Population",
        vColor = "Income",
        type= "value",
        bg.labels = "red",
        title = "미국")
#미국은 소득이 전반적으로 균등한것같다. 
...
# 시오.
symbols(us1$Income,us1$Illiteracy,
        circles = us1$Population,
        inches = 0.5,
        fg="red",
        bg="yellow",
        lwd=1.5,
        ...
        ylab = "Illiteracy",
        main = "소득과 문맹률")
text(us1$Income,us1$Illiteracy,
     rownames(us1),
     cex = 0.6,
     col="black")
        
#음의 상관관계가 있다. 인구가 많은 지역이 문맹률이 낮다.        
...
symbols(us1$Illiteracy,us1$Murder,
        circles =us1$Area,
        inches = 0.5,
        fg="red",
        bg="yellow",
        ...
        ylab = " Murder",
        main = "문맹률과 범죄율")
text(us1$Illiteracy,us1$Murder,
     rownames(us1),
     cex = 0.6,
     col="black")
